// 采集知乎回答原始富文本 HTML（含配图 <img>、加粗 <strong>、列表、公式图等）
// 与 zhihu-fetch-content.mjs 不同：本脚本【不】对 content 做 stripHtml，保留 data.content 原始 HTML，
// 存入 rec.contentHtml 字段，供前端按原网页排版渲染（含配图）。
// 速率：每条间睡 GAP；每 CHUNK 条后睡 CHUNK_SLEEP（等限频窗口）；失败 45s 退避重试 3 次，最终失败记 null 跳过。
// 断点续跑：已有 contentHtml 的记录自动跳过，可反复重跑补齐失败的。
...
let items = db.items || [];
let todo = items.filter(r => !r.contentHtml || (r.contentHtml === ''));
if (LIMIT > 0) todo = todo.slice(0, LIMIT);
console.log(`[start] 待补HTML: ${todo.length} 条 | 已有HTML ${items.filter(r => r.contentHtml).length}/${items.length} | gap=${GAP} chunk=${CHUNK} sleep=${CHUNK_SLEEP}`);

let done = 0, fail = 0, imgTotal = 0;
for (let i = 0; i < todo.length; i += CHUNK) {
  const batch = todo.slice(i, i + CHUNK);
  console.log(`\n=== 批次 ${Math.floor(i / CHUNK) + 1} (${i + 1}..${Math.min(i + CHUNK, todo.length)})/${todo.length} ===`);
  for (const rec of batch) {
    const data = await fetchWithRetry(['answer', rec.contentId], 'HTML');
    if (data && data.content) {
      rec.contentHtml = data.content;
      rec.imgCount = (data.content.match(/<img/gi) || []).length;
      rec.htmlLen = data.content.length;
      imgTotal += rec.imgCount;
      done++;
      console.log(`  ✓ ${rec.contentId} len=${rec.htmlLen} img=${rec.imgCount}`);
    } else { fail++; console.log(`  ✗ ${rec.contentId} 失败`); }
    await sleep(GAP);
    save(items, `batch ${Math.floor(i / CHUNK) + 1}`);
  }
  if (i + CHUNK < todo.length) { console.log(`[chunk sleep] ${CHUNK_SLEEP}s 等窗口...`); await sleep(CHUNK_SLEEP * 1000); }
}
console.log(`\n[done] HTML补齐 ${done}，失败 ${fail}；累计有HTML ${items.filter(r => r.contentHtml).length}/${items.length}；图片总数 ${imgTotal}`);
save(items, 'fetch-html complete');
