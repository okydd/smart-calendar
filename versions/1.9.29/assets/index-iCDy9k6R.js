...
${s.description?"备注："+s.description:""}`;if(e){const f=l.unit==="day"?"天":l.unit==="hour"?"小时":"分钟";try{const{fireReminderNow:x}=await Bn(async()=>{const{fireReminderNow:g}=await import("./localNotify-DyTtRami.js");return{fireReminderNow:g}},__vite__mapDeps([0,1,2,3,4,5]),import.meta.url);await x(`⏰ ${s.title}`,`${r.month()+1}月${r.date()}日 ${d}（提前${l.value}${f}提醒）`+(s.description?`
